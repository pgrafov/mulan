...
class smsapi:
	...
	def balance(self):
		resp = self._communicate({'method':'get_profile'}, self.cookie)
		if resp[0][0] != 0:
			raise smsapi_other_exception(resp[0][0])
		credits = float(resp[1]['credits'])
		credits_name = resp[1]['credits_name']
		return '%s %s' % (credits, credits_name)
	...
	def push_msg(self, text, phone, sender_name = None, Unicode = None,
		...
		req = {"method":"push_msg", "text":text, "phone":phone, 'api_v': '1.1',  'satellite_adv' : 'IF_EXISTS'}
